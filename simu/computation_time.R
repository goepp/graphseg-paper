...
## Method with neth-neigh [DEPRECATED] ------
# load("synthetic/neth_neigh.RData")
# 
# neth_baseline <- data.frame("district_code" = levels(neth_neigh$district_code),
#                             "baseline" = rpois(nlevels(neth_neigh$district_code), 10))
# neth_neigh <- neth_neigh %>% left_join(neth_baseline, by = "district_code")
# 
# nrep <- 10
...
# 
# set.seed(92)
# neth_gamma_array <- replicate(
#   nrep, sapply(sigma_sq, function(sigma_sq) neth_neigh$baseline + rnorm(nrow(neth_neigh), 0, sigma_sq))
# )
# ind_sigma <- 1
# # neth_neigh_bench <- microbenchmark(
# #   res = agraph(neth_gamma_array[, ind_sigma, 1], graph_neth_neigh, lambda),
# #   res2 = flsa_graph(neth_gamma_array[, ind_sigma, 1], graph_neth_neigh, lambda),
# #   times = 1L
# # )
# 
# ## utrecht_neigh: n = 2955
# load("synthetic/utrecht_neigh.RData")
# 
# utrecht_baseline <- data.frame("district_code" = levels(utrecht_neigh$district_code),
#                                "baseline" = rpois(nlevels(utrecht_neigh$district_code), 10))
# utrecht_neigh <- utrecht_neigh %>% left_join(utrecht_baseline, by = "district_code")
# 
# nrep <- 10
# sigma_sq <- c(seq(0.1, 1.2, 0.1), 2, 3, 5)
# names(sigma_sq) <- sigma_sq
...
# # utrecht_neth_neigh_bench <- microbenchmark(
# #   "agraph_neth" = agraph(neth_gamma_array[, ind_sigma, 1], graph_neth_neigh, lambda),
# #   "agraph_utrecht" = agraph(utrecht_gamma_array[, ind_sigma, 1], graph_utrecht_neigh, lambda),
# #   "flsa_neth" = flsa_graph(neth_gamma_array[, ind_sigma, 1], graph_neth_neigh, lambda),
# #   "flsa_utrecht" = flsa_graph(utrecht_gamma_array[, ind_sigma, 1], graph_utrecht_neigh, lambda),
# #   times = 5L
# # )
# utrecht_neigh_bench %>% plot
# 
# neigh_bench <- readRDS(file = "simu_results/neigh_bench.rds") [END DEPRECATED]

## Method with automatically extracted subgraphs -----
load(file = "simu/synthetic/graph_sub.RData")
...
# bench_df <- mapply(bench_agraph_flsa,
#                    list(sub_100$signal, sub_300$signal, sub_1000$signal),
#                    list(graph_sub_100, graph_sub_300, graph_sub_1000),
#                    MoreArgs = list(lambda = lambda, time = n_times),
...
bench_df <- readRDS(file = "simu/simu_results/bench_df_cluster.rds")
bench_margin_df <- bench_df %>%
  mutate(sample_size = rep(c(100, 300, 1000, 3000, 10000), each = 2 * n_times),
         time = time / 1e9) %>%
...
bench_df <- readRDS(file = "simu/simu_results/bench_df_cluster.rds")
bench_df_for_graph <- bench_df %>%
  mutate(sample_size = rep(c(100, 300, 1000, 3000, 10000), each = 2 * n_times),
         time = time / 1e9) %>%
  dplyr::group_by(expr, sample_size)

ggplot(bench_df_for_graph, aes(as.factor(sample_size), time, color = expr)) +
  geom_boxplot() +
  theme_minimal() +
  scale_color_discrete(labels = c("adaptive ridge", "flsa")) +
  ylab("Computing time (seconds)") + 
  xlab(TeX("Number of areas (p)")) + 
  # scale_y_log10() +
...
bench_df_crossbar <- bench_df_for_graph %>% 
  group_by(sample_size, expr) %>% 
  summarize(mean = mean(time), std = sd(time), .groups = "drop") %>% 
  mutate(lower = mean - std, upper = mean + std)

ggplot(bench_df_crossbar, aes(x = sample_size, y = mean, ymin = lower,
...
width_pdf <- 6
height_pdf <- 4
ggsave(file = "simu/figure/computing_time.pdf", 
       device = cairo_pdf,
       width = width_pdf, height = height_pdf)
