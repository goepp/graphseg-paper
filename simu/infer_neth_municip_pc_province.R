...
library(kableExtra); library(ggpubr); library(mclust)
library(dendextend)
...
neth_municip = readRDS("simu/synthetic/neth_municip.rds")
graph_neth_municip <- readRDS("simu/synthetic/graph_neth_municip.rds")


# Load data, simulate data ---------------------------------------------
set.seed(87)
baseline <- data.frame("province_code" = levels(neth_municip$province_code),
                       "baseline" = rpois(nlevels(neth_municip$province_code), 10))
neth_municip <- neth_municip %>% left_join(baseline, by = "province_code") %>% 
  mutate(true_labels = province_code %>% as.factor %>% as.numeric)
...
gamma_array <- replicate(
  nrep, sapply(sigma_sq, function(sigma_sq) neth_municip$baseline + rnorm(nrow(neth_municip), 0, sigma_sq))
)
rms0 <- sapply(seq_along(sigma_sq), function(a)
  sqrt(sum((gamma_array[, a, 1] - neth_municip$baseline) ^ 2 ) / length(gamma))
)
...
# Agraph
full_infer_res <- apply(gamma_array[, , 1], 2, agraph, graph = graph_neth_municip,
                        lambda = lambda, weights = NULL, shrinkage = TRUE, tol = 1e-8, itermax = 50000)
rms_path <- lapply(full_infer_res, function(a)
  sqrt(rowSums((a$result - t(replicate(length(lambda), neth_municip$baseline))) ^ 2) / length(gamma)))
...
clust_score_path <- lapply(full_infer_res, function(z) {
  apply(z$result, 1, function(zz) {
    score <- score(as.numeric(as.factor(zz)), neth_municip$true_labels)
    return(ifelse(is.nan(score), 0, score))
  }) %>% unname
})
# Flsa
full_infer_res_flsa <- apply(gamma_array[, , 1], 2, flsa_graph, graph = graph_neth_municip,
                             lambda = lambda)
...
clust_score_vs_model_dim <- ggplot() +
  ...
  labs(title = "How well each method recovers the true zones", tag = "(a)") +
  theme_minimal() + ylim(c(0, 1)) +
  # theme(text = element_text(size = 10, family = "LM Roman 10")) +
  ylab("Fowlkes-Mallows index") + xlab(TeX("Model dimension e($\\lambda$)"))
...
rms_vs_model_dim <- ggplot() +
  ...
  # theme(text = element_text(size = 10, family = "LM Roman 10")) +
  labs(title = "How well each method recovers the true signal", tag = "(b)")

clust_score_rms_plot <- ggarrange(
  clust_score_vs_model_dim, rms_vs_model_dim,
  common.legend = TRUE, legend = "bottom")
ggexport(clust_score_rms_plot, filename = "simu/figure/utrecht_neigh_pc_district/utrecht_neigh_pc_district_rms_model_dim_crit.pdf",
         width = 6 * 0.7 * 2, height = 11 * 0.7)
ggexport(clust_score_rms_plot, filename = "../paper/jan_vivien_segmentation/figure/utrecht_neigh_pc_district_rms_model_dim_crit.pdf",
         width = 6 * 0.7 * 2, height = 11 * 0.7)

#### Regularization plot for any sigma_sq -------------------------------------
ind = 10
# Wrt model dimensions
...
## Inference ---------------------------------------------------------------
infer_res <- apply(gamma_array, c(2, 3), infer, graph = graph_neth_municip, lambda = lambda,
                   baseline = neth_municip$baseline, 
                   true_labels = neth_municip$true_labels)
infer_res_flsa <- apply(gamma_array, c(2, 3), infer_flsa, graph = graph_neth_municip, lambda = lambda,
                        baseline = neth_municip$baseline, 
                        true_labels = neth_municip$true_labels)
...
#      file = "simu/simu_results/infer_neth_municip_pc_province.RData")
# load(file = "simu/simu_results/infer_neth_municip_pc_province.RData")
...
#         file = "simu/simu_results/neth_municip_pc_province_dim_rms_clust_score_df.rds")
...
saveRDS(tidy_df, file = "simu/simu_results/neth_municip_pc_province_tidy_df.rds")
...
saveRDS(eee, file = "simu/table/neth_municip_pc_province/neth_municip_pc_province_rms_dim_df.rds")
saveRDS(ddd, file = "simu/table/neth_municip_pc_province/neth_municip_pc_province_rms_dim_kable.rds")
ddd %>% cat(file = "simu/table/neth_municip_pc_province/neth_municip_pc_province_rms_dim_table.tex")
...
prod_table_neth_municip_pc_province <- aaa %>% reshape2::acast(sigma_sq ~ criterion + treatment) %>%
  as.data.frame(stringsAsFactors = FALSE) %>%
  rownames_to_column(var = linebreak("Noise \n standard \n deviation")) %>%
  mutate("no_reg" = paste('$', sprintf("%.3f", round(rms0, digits = 4)), '$'))
for (ind1 in 1:ncol(min_col)) {
  for (ind2 in 1:nrow(min_col)) {
    prod_table_neth_municip_pc_province[ind1, min_col[ind2, ind1] + 1] <-
      paste0("\\boldmath{",
             prod_table_neth_municip_pc_province[ind1, min_col[ind2, ind1] + 1],
             "}")
  }
}
bbb <- prod_table_neth_municip_pc_province %>%
  kable("latex", booktabs = TRUE, escape = FALSE,
        col.names = linebreak(c("Noise \n standard \n deviation",
                                rep(c("Adaptive \n Ridge", "FLSA"), 3),
                                "No \n Regula- \n rization"), align = "c")
...
bbb %>% cat(file = "simu/table/neth_municip_pc_province/neth_municip_pc_province_rms_table.tex")


# Plot results
## RMS
# AIC is the best criterion for small noise, and gcv for big noise
# Ardige vs FLSA
...
ggplot(data = rms_df %>% filter(criterion != "gcv"),
  ...
  stat_summary(fun = median, geom = "line", aes(group = treatment))
ggplot(data = rms_df,
       aes(x = as.numeric(as.character(sigma_sq)),
  ...
  theme(legend.position = c(0.15, 0.85)) +
  scale_y_log10() + scale_x_log10() +
  stat_summary(fun = median, geom = "line", aes(group = criterion))

ggplot(data = rms_df %>% filter(criterion == "aic"),
       aes(x = as.numeric(as.character(sigma_sq)),
           y = rms, group = interaction(as.numeric(as.character(sigma_sq)), treatment),
           color = treatment)) +
  geom_boxplot() +
  theme_bw() + ylab("Root Mean Square") + xlab("Noise standard deviation") +
  theme(legend.position = c(0.15, 0.85)) + ggtitle("Model with AIC") +
  scale_y_log10() + scale_x_log10() +
  stat_summary(fun = median, geom = "line", aes(group = treatment))


#### previous plot: agraph only -----------------------------------------
...
for (ind_sigma in seq_along(sigma_sq)) {
  ...
  res <- agraph(gamma = gamma_array[, ind_sigma, 1],
                ...
                tol = 1e-8,
                itermax = 50000)
  connlist <- graph2connlist(graph_utrecht_neigh)
  ...
  utrecht_neigh$theta <- res$result[which.min(res$bic), ]
  utrecht_neigh$theta_flsa <- res2[which.min(res$bic), ]
  ...
  ggsave(file = paste0("simu/figure/utrecht_neigh/pc/utr_neigh_pc_reg_path_sigma",
  ...
  ggplot(data = res_path_flsa, aes(lambda, value, group = area)) +
    ...
    geom_vline(xintercept = lambda[sapply(res[c("aic", "bic", "gcv")], which.min)], col = "red") +
    theme(legend.position = "none")
  ggsave(file = paste0("simu/figure/utrecht_neigh/pc/utr_neigh_pc_reg_path_flsa_sigma",
  ...
  # plot(st_geometry(utrecht_neigh), add = TRUE, lwd = 0.2, border = "grey")
  # par(op)
  # dev.copy2pdf(file = paste0("figure/utrecht_neigh/pc/utr_neigh_pc_baseline_vs_seg_sigma",
  ...
  plot(st_geometry(utrecht_neigh), add = TRUE, lwd = 0.2, border = "grey")
  par(op)
  dev.copy2pdf(file = paste0("simu/figure/utrecht_neigh/pc/utr_neigh_pc_baseline_seg_overlay_sigma",
                             ind_sigma,".pdf"), width = width_pdf, height = height_pdf)
  ...
  par(op)
  dev.copy2pdf(file = paste0("simu/figure/utrecht_neigh/pc/utr_neigh_pc_unseg_sigma",
                             ind_sigma,".pdf"), width = width_pdf, height = height_pdf)
  ...
  plot(st_geometry(utrecht_neigh), col = NA, add = TRUE,
       main = "", lwd = 0.2, border = alpha("black", 0.6))
  par(op)
  dev.copy2pdf(file = paste0("simu/figure/utrecht_neigh/pc/utr_neigh_pc_baseline_sigma",
                             ind_sigma,".pdf"), width = width_pdf, height = height_pdf)
  ...
  par(op)
  dev.copy2pdf(file = paste0("simu/figure/utrecht_neigh/pc/utr_neigh_pc_seg_sigma",
                             ind_sigma,".pdf"), width = width_pdf, height = height_pdf)
}


# Visual inspection: what is the signal-to-noise ratio?
plot(gamma_array[,3,1])
