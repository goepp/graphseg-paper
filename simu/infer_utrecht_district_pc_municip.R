...
utrecht_district <- readRDS("simu/synthetic/utrecht_district.rds")

# Load data, simulate data ---------------------------------------------
set.seed(87)
baseline <- data.frame("municip_code" = levels(utrecht_district$municip_code),
                       "baseline" = rpois(nlevels(utrecht_district$municip_code), 10))
utrecht_district <- utrecht_district %>% left_join(baseline, by = "municip_code") %>% 
...
gamma_array <- replicate(
  nrep, sapply(sigma_sq, function(sigma_sq) utrecht_district$baseline +
                 rnorm(nrow(utrecht_district), 0, sigma_sq))
)
rms0 <- sapply(seq_along(sigma_sq), function(a)
  sqrt(sum((gamma_array[, a, 1] - utrecht_district$baseline) ^ 2 ) / length(gamma))
)
...
# Aridge
full_infer_res <- apply(gamma_array[, , 1], 2, agraph, graph = graph_utrecht_district,
                        lambda = lambda, weights = NULL, shrinkage = TRUE, tol = 1e-8, itermax = 50000)
rms_path <- lapply(full_infer_res, function(a)
  sqrt(rowSums((a$result - t(replicate(length(lambda), utrecht_district$baseline))) ^ 2) / length(gamma)))
dim_path <- lapply(full_infer_res, function(a) a$model_dim)
# Flsa
full_infer_res_flsa <- apply(gamma_array[, , 1], 2, flsa_graph, graph = graph_utrecht_district,
                             lambda = lambda)
...
criterion_df <- bind_rows(
  ...
  bind_cols(list("aic" = lapply(full_infer_res, function(a) a$aic) %>% unlist,
    ...
    dplyr::mutate("lambda_" = rep(lambda, length(sigma_sq)),
                  ...
                  "treatment" = "flsa"))
path_df <- bind_rows(
  dplyr::left_join(gather(data.frame(dim_path), key = "sigma_sq", value = "model_dim"),
             gather(data.frame(rms_path), key = "sigma_sq", value = "rms"),
             by = "sigma_sq") %>% ## ERROR: left_join repeats the matrix (hadamard product-style)
    as_tibble() %>% 
...
ggsave(file = "simu/figure/utrecht_district_pc_municip/utrecht_district_pc_municip_rms_model_dim_crit.pdf",
       width = 6, height = 11)
...
ggplot(path_df %>% filter(Noise_sd %in% c(0.5, 0.8, 1.0)),
  ...
  ylab("Root mean square") + xlab("Model dimension") +
  scale_x_log10() + scale_y_log10()
# ggsave(file = "simu/figure/utrecht_district_pc_municip/utrecht_district_pc_municip_rms_path_model_dim.pdf",
#        width = 6, height = 11)
...
# infer_res <- apply(gamma_array, c(2, 3), infer, graph = graph_utrecht_district,
#                    lambda = lambda, baseline = utrecht_district$baseline,
#                    true_labels = utrecht_district$true_labels)
# infer_res_flsa <- apply(gamma_array, c(2, 3), infer_flsa, graph = graph_utrecht_district,
#                         lambda = lambda, baseline = utrecht_district$baseline,
#                         true_labels = utrecht_district$true_labels)
...
#      file = "simu/simu_results/infer_utrecht_district_pc_municip.RData")
# load(file = "simu/simu_results/infer_utrecht_district_pc_municip.RData")
...
#         file = "simu/simu_results/utrecht_district_pc_municip_dim_rms_clust_score_df.rds")
...
saveRDS(tidy_df, file = "simu/simu_results/utrecht_district_pc_municip_tidy_df.rds")
...
saveRDS(eee, file = "simu/table/utrecht_district_pc_municip/utrecht_district_pc_municip_rms_dim_df.rds")
saveRDS(ddd, file = "simu/table/utrecht_district_pc_municip/utrecht_district_pc_municip_rms_dim_kable.rds")
ddd %>% cat(file = "simu/table/utrecht_district_pc_municip/utrecht_district_pc_municip_rms_dim_table.tex")
...
bbb %>% cat(file = "simu/table/utrecht_district_pc_municip/utrecht_neigh_pc_rms_table.tex")
