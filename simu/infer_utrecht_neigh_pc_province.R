...
set.seed(87)
baseline <- data.frame("province_code" = unique(utrecht_neigh$province_code),
                       "baseline" = rpois(length(unique(utrecht_neigh$province_code)), 10))
utrecht_neigh <- utrecht_neigh %>% left_join(baseline, by = "province_code") %>% 
...
criterion_df <- bind_rows(
  ...
  bind_cols(list("aic" = lapply(full_infer_res, function(a) a$aic) %>% unlist,
    ...
    dplyr::mutate("lambda_" = rep(lambda, length(sigma_sq)),
                  ...
                  "treatment" = "flsa"))
path_df <- bind_rows(
  bind_cols(gather(data.frame(dim_path), key = "sigma_sq1", value = "model_dim"),
            gather(data.frame(rms_path), key = "sigma_sq2", value = "rms")) %>%
    as_tibble() %>% 
    mutate("Noise_sd" = as.numeric(str_remove(sigma_sq1, "X"))) %>%
    select(-sigma_sq1, -sigma_sq2) %>%
    mutate("lambda_" = rep(lambda, length(sigma_sq))) %>%
    mutate("treatment" = "aridge"),
  bind_cols(gather(data.frame(dim_path_flsa), key = "sigma_sq1", value = "model_dim"),
            gather(data.frame(rms_path_flsa), key = "sigma_sq2", value = "rms")) %>%
    mutate("Noise_sd" = as.numeric(str_remove(sigma_sq1, "X"))) %>%
    select(-sigma_sq1, -sigma_sq2) %>%
    mutate("lambda_" = rep(lambda, length(sigma_sq))) %>%
...
ggsave(file = "figure/utrecht_neigh_pc_province/utrecht_neigh_pc_province_rms_model_dim_crit.pdf",
       width = 6, height = 11)
...
ggplot(path_df %>% filter(Noise_sd %in% c(0.5, 0.8, 1.0)),
  ...
  ylab("Root mean square") + xlab("Model dimension") +
  scale_x_log10() + scale_y_log10()
ggsave(file = "figure/utrecht_neigh_pc_province/utrecht_neigh_pc_province_rms_path_model_dim.pdf",
       width = 6, height = 11)
...
#      file = "simu/simu_results/infer_utrecht_neigh_pc_province_dim_rms_clust_score.RData")
# load(file = "simu/simu_results/infer_utrecht_neigh_pc_province_dim_rms_clust_score.RData")
### As on dataframe ----------------------------------------------------
...
#         file = "simu/simu_results/utrecht_neigh_pc_province_dim_rms_clust_score_df.rds")
...
saveRDS(tidy_df, file = "simu/simu_results/utrecht_neigh_pc_province_tidy_df.rds")

## Table results
# Rms and dim table with AIC criterion
ccc <- dim_rms_df %>% 
...
saveRDS(eee, file = "table/utrecht_neigh_pc_province/utrecht_neigh_pc_province_rms_dim_df.rds")
saveRDS(ddd, file = "table/utrecht_neigh_pc_province/utrecht_neigh_pc_province_rms_dim_kable.rds")
ddd %>% cat(file = "table/utrecht_neigh_pc_province/utrecht_neigh_pc_province_rms_dim_table.tex")
...
bbb %>% cat(file = "table/utrecht_neigh/pc/utrecht_neigh_pc_rms_table.tex")
...
ggplot(data = rms_df, aes(x = as.numeric(as.character(sigma_sq)), y = rms, group = sigma_sq)) +
  ...
  scale_y_log10() + scale_x_log10()
ggsave(file = "figure/utr_neigh_pc_rms.pdf", width = 6, height = 5)
...
ggplot(data = dim_df %>% filter(criterion != "gcv"),
  ...
  scale_y_log10()
ggsave(file = "figure/utr_neigh_pc_dim.pdf", width = 6, height = 5)
...
for (ind_sigma in seq_along(sigma_sq)) {
  ...
  ggsave(file = paste0("figure/utrecht_neigh/pc/utr_neigh_pc_reg_path_sigma",
  ...
  ggplot(data = res_path_flsa, aes(lambda, value, group = area)) +
    ...
    geom_vline(xintercept = lambda[sapply(res[c("aic", "bic", "gcv")], which.min)], col = "red") +
    theme(legend.position = "none")
  ggsave(file = paste0("figure/utrecht_neigh/pc/utr_neigh_pc_reg_path_flsa_sigma",
