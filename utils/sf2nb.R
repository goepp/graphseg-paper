...
sf2nb <- function(x, queen = TRUE, sparse = TRUE) {
	# x      = sf object
	# queen  = logical, queen-type (TRUE) of rook-type (FALSE) adjacencies
	# sparse = logial, return list (TRUE) or matrix (FALSE)
	...
	if (queen) {
		# Queen-type
		x.adj <- st_relate(x, x, pattern = "F***T****", sparse = FALSE)
	} else {
		# Rook-type
	...
	while(x.comp$no > 1) {
		...
		dist.subgraph <- matrix(Inf, nrow = x.comp$no, ncol = x.comp$no)
